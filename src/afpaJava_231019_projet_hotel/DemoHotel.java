...
public class DemoHotel {
	...
	public static void tests() {
		...
		for (String type: listeType) {
			String res = Chambre.numPremiereChambreVide(type) == 999 ? "Aucune chambre libre" : Integer.toString(Chambre.numPremiereChambreVide(type));
		...
		System.out.println("Réserver première chambre vide type single (n° " + Chambre.numPremiereChambreVide("single") + ") :");
		Chambre.reserverChambre(Chambre.numPremiereChambreVide("single"));
		Hotel.afficherEtatHotel();
		System.out.println(("---"));
		
		// Libérer dernière chambre occupée du type "single"
		System.out.println("Libérer la dernière chambre vide type single (n° " + Chambre.numDerniereChambreOccupee("single") + ") :");
